...
namespace AriaPlugin.Runtime.GifMaker.Encoder
{
	public class GifEncoder
	{
		...
		protected System.Action<byte[]> onFinishHandler = null;
		...
		public void Start(System.Action<byte[]> onFinish )
		{
		...
		public void Start(String file)
		{
			try
			{
				m_FileStream = new FileStream(file, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None);
				Start((bytes) =>  { m_FileStream.Write(bytes, 0, bytes.Length); });
		...
		public void Finish()
		{
			...
			try
			{
				...
				var bytes = new byte[(int)m_MemoryStream.Length];
				m_MemoryStream.Position = 0;
				m_MemoryStream.Read(bytes, 0, (int)m_MemoryStream.Length);
				m_MemoryStream.Flush();

				onFinishHandler(bytes);
		...
		protected void WriteGraphicCtrlExt()
		{
			...
			m_MemoryStream.WriteByte(Convert.ToByte(0)); // Transparent color index
